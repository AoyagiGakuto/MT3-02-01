...
// 球同士の衝突判定
bool IsSphereCollision(const Sphere& a, const Sphere& b)
{
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    // 球を2つ作る
    Sphere sphereA = { { -1.0f, 0.0f, 0.0f }, 0.2f };
    Sphere sphereB = { { 1.0f, 0.0f, 0.0f }, 0.2f };
    ...
    while (Novice::ProcessMessage() == 0) {
        ...
        // 衝突判定
        bool isHit = IsSphereCollision(sphereA, sphereB);
        ...
        ImGui::DragFloat3("SphereA Pos", &sphereA.center.x, 0.01f);
        ImGui::DragFloat("SphereA Radius", &sphereA.radius, 0.01f);
        ImGui::DragFloat3("SphereB Pos", &sphereB.center.x, 0.01f);
        ...
        // 衝突してたら赤、してなかったら白
        uint32_t colorA = isHit ? RED : WHITE;
        uint32_t colorB = isHit ? RED : WHITE;

        DrawSphere(sphereA, viewProjectionMatrix, viewportMatrix, colorA);
        DrawSphere(sphereB, viewProjectionMatrix, viewportMatrix, colorB);
